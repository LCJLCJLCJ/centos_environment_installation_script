...
# 变量定义
newMongodbTar="mongodb-linux-x86_64-3.0.1.tgz"
tarPkgPath="/home/muzi/winShared/Scripts/pkg/"
# 校验安装包是否存在
check_pkg_sh=`echo $tarPkgPath |awk -F "pkg/" '{print $1}'`"check_pkg.sh"
sh $check_pkg_sh $tarPkgPath$newMongodbTar
if [ $? -ne 0 ];then
	echo "pkg file:[ $newMongodbTar ] is ok"
else
	echo "Error: pkg file:[ $newMongodbTar ] error, please confirm it"
	exit 0
fi


# 安装Mongodb
tar zxf $tarPkgPath$newMongodbTar -C /usr/local/
